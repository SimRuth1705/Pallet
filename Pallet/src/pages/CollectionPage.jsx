...
function CollectionPage() {
  ...
  useEffect(() => {
    ...
    if (params.gender) {
      productsCopy = productsCopy.filter(
        (item) => item.gender.toLowerCase() === params.gender.toLowerCase()
      );
    }
    ...
    if (params.color) {
      productsCopy = productsCopy.filter(
        (item) =>
          item.colors &&
          item.colors.some(
            (c) => c.toLowerCase() === params.color.toLowerCase()
          )
      );
    }

    if (params.size) {
      const selectedSizes = params.size.split(",");
      productsCopy = productsCopy.filter(
        (item) =>
          item.sizes &&
          item.sizes.some((s) => selectedSizes.includes(s.toLowerCase()))
      );
    }
    ...
    if (params.material) {
      ...
      productsCopy = productsCopy.filter(
        (item) =>
          item.material &&
          selectedMaterials.includes(item.material.toLowerCase())
      );
    }
